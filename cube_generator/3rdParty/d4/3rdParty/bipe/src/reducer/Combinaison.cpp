...
void Combinaison::run(unsigned nbVar, std::vector<std::vector<Lit>> &clauses,
  ...
  unsigned nbUnit = 0, nbBin = 0, nbTernary = 0, nbOther = 0, nbLits = 0;
  for (auto &cl : result) {
    switch (cl.size()) {
      case 1:
        nbUnit++;
  ...
  std::cout << "c Number of clauses: " << result.size() << '\n';
  std::cout << "c Number of units: " << nbUnit << '\n';
  std::cout << "c Number of binaries: " << nbBin << '\n';
  std::cout << "c Number of ternaries: " << nbTernary << '\n';
  std::cout << "c Number of clause of size greater than 3: " << nbOther << '\n';
  std::cout << "c Number of literals: " << nbLits << '\n';
}  // run
